    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          unitId,
          // 實際上應該是上傳後的檔案URL
          photo: uploadedFiles.photo?.name,
          coachCertificate: uploadedFiles.coachCertificate?.name,
          consentForm: uploadedFiles.consentForm?.name
        })
      });
      ...
      <div className="mb-8">
        ...
        {athletes.length > 0 && (
          <div className="mb-6 p-4 bg-blue-50 rounded-lg">
            <p className="text-sm text-blue-700 font-medium mb-3">已註冊選手：{athletes.length} 位</p>
            <div className="space-y-2">
              {athletes.map((athlete, index) => (
                <div key={athlete.id} className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm">
                  <div className="flex-1">
                    <div className="text-sm text-gray-900 font-medium">
                      {index + 1}. {athlete.name} - {athlete.gender === 'M' ? '男' : '女'} - {athlete.weight}kg
                    </div>
        ...
        <div className="border-t pt-6">
          ...
          <div className="mb-4 p-3 bg-gray-50 rounded-lg">
            ...
            <div className="flex flex-wrap gap-3 text-xs">
              ...
              {ageInfo && ageInfo.ageGroup !== 'adult' && ageInfo.ageGroup !== 'master' && (
                <span className={`px-2 py-1 rounded font-medium ${uploadedFiles.consentForm ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-800'}`}>
                  紙本同意書 {uploadedFiles.consentForm ? '✓' : '(可選)'}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="space-y-6">
            <FileUpload
              label="選手照片 *"
              description="請上傳選手近照，支援拖曳或拍照上傳 (限制：1個檔案，最大10MB)"
              accept="image/*"
            ...
            <FileUpload
              ...
              allowCamera={true}
              onFileSelect={(file) => {
                setUploadedFiles(prev => ({ ...prev, coachCertificate: file }));
                setError(null); // 清除之前的錯誤
              }}
            />
            ...
            {ageInfo && ageInfo.ageGroup !== 'adult' && ageInfo.ageGroup !== 'master' && (
              <FileUpload
                ...
                description="未滿18歲選手可額外上傳監護人簽名的紙本同意書 (限制：1個檔案，最大10MB)"
                accept="image/*,application/pdf"
                maxSize={10 * 1024 * 1024}
                allowCamera={true}
                onFileSelect={(file) => {
                  setUploadedFiles(prev => ({ ...prev, consentForm: file }));
                  setError(null); // 清除之前的錯誤
                }}
              />
            )}
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="text-sm text-gray-900 space-y-2 mb-4">
                ...
                <p className="mt-3 text-gray-800 font-medium">
                  <strong className="text-red-700">注意：</strong>未滿18歲選手需由監護人代為簽名同意。
                </p>
              </div>
