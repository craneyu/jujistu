/**
 * 建立預設競賽項目與分組（原 prisma/seed.ts 的 CommonJS 版本）。
 *
 * 改寫成純 JS 的原因：原本透過 `prisma db seed` 執行，會用 ts-node 即時編譯
 * TypeScript，需要 ts-node 與 typescript 兩個 devDependency，且命令字串裡的
 * `--compiler-options {"module":"CommonJS"}` 還要經過 shell 引號處理。
 * 這在部署環境既慢又脆弱，直接用 node 執行可以完全避開。
 */
...
const eventTypes = [
  { key: 'fighting', name: '對打', description: '傳統柔術對打項目', requiresTeam: false },
  { key: 'newaza', name: '寢技', description: '地面技術競賽', requiresTeam: false },
  { key: 'fullcontact', name: '格鬥', description: '全接觸格鬥項目', requiresTeam: false },
  { key: 'duo_traditional', name: '傳統演武', description: '傳統套路雙人演武', requiresTeam: true },
  { key: 'duo_creative', name: '創意演武', description: '創意套路雙人演武', requiresTeam: true },
  { key: 'nogi', name: '無道袍', description: '不穿道袍的競技項目', requiresTeam: false },
];
